...
alt.onServer('auth:showLoginForm', () => {
    if (!authBrowser) {
        ...
        authBrowser.on('auth:tryLogin', (username, password) => {
            alt.emitServer('auth:tryLogin', username, password);
        });

        authBrowser.on('auth:tryRegister', (username, password) => {
            alt.emitServer('auth:tryRegister', username, password);
        });
